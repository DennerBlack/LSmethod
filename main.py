...
def vis(data, images_number, img_index, name):
    ...
    sum_of_summ_of_measure_groups = np.sum((np.asarray([np.mean(y[i:data_width*(i+1)]) for i in range(data_len)]) - my)**2)
...
def vis_poly(data, images_number, img_index, name):
    d = data
    x = d['X']
    y = d['Y']

    poly_reg = np.poly1d(np.polyfit(x, y, 2))
    ...
    sdpa = np.sum((x - mx)*(y - my))
    ssdy = np.sum((y - my)**2)
    ssdy_reg = np.sum((poly_reg(x) - mx)**2)
    ssd_lin_real = np.sum((y - poly_reg(x))**2)
    s2 = ssd_lin_real/(all_data_num-2)
    alpha = 0.05
    s = np.sqrt(s2)
    t_diff = t.interval(1. - alpha, all_data_num-2)
    min_int = [poly_reg(x)[i*data_width] + t_diff[0] * np.sqrt(1 + 1/all_data_num + (((x[i*data_width] - mx)**2)/ssdx))*s for i in range(data_len)]
    max_int = [poly_reg(x)[i*data_width] + t_diff[1] * np.sqrt(1 + 1/all_data_num + (((x[i*data_width] - mx)**2)/ssdx))*s for i in range(data_len)]
    sum_of_summ_of_measure_groups = np.sum((np.asarray([np.mean(y[i:data_width*(i+1)]) for i in range(data_len)]) - my)**2)
    ss_diff = (ssdy-ssdy_reg)/sum_of_summ_of_measure_groups
    ...
    dot_element_sdpa = Line2D([0], [0], marker='o', color='w', label=r'$\sum\left(x_i-\bar{x}\right)\left(y_i-\bar{y}\right)$ = '+f'{sdpa:.3f}',markerfacecolor='black', markersize=6)
    dot_element_ssdy = Line2D([0], [0], marker='o', color='w', label=r'$\sum\left(y_i-\bar{y}\right)^2$ = '+f'{ssdy:.3f}',markerfacecolor='black', markersize=6)
    dot_element_lr_formula = Line2D([0], [0], marker='o', color='w', label='Общий вид полиномиальной регрессии:\n'+r'$\hat{y} = \beta_0 + \beta_1 x + \beta_2 x^2 + \beta_3 x^3 + \cdots + \beta_n x^n + \varepsilon. $',markerfacecolor='black', markersize=6)
    #dot_element_coeffs = Line2D([0], [0], marker='o', color='w', label=r'a = '+f'{a:.3f}'+r'; b = '+f'{b:.3f}'+r'; $\varepsilon$ = 0',markerfacecolor='black', markersize=6)
    dot_element_ssdy_reg = Line2D([0], [0], marker='o', color='w', label=r'$\sum\left(\hat{y}-\bar{y}\right)^2$ = '+f'{ssdy_reg:.3f}',markerfacecolor='black', markersize=6)
    dot_element_s2 = Line2D([0], [0], marker='o', color='w', label=r'$s^2=\frac{\sum\left(y_i-\hat{y}\right)^2}{n-2}$ = '+f'{s2:.3f}',markerfacecolor='black', markersize=6)
    dot_element_s = Line2D([0], [0], marker='o', color='w', label=r'S = '+f'{s:.3f}',markerfacecolor='black', markersize=6)
...
vis(synth_data_linear, 4, 1, "Линейновозрастающий набор данных c наложенным Гауссовым шумом")
vis(synth_data_sin, 4, 2, "Синусоидный набор данных c наложенным Гауссовым шумом")
vis(synth_data_expanse, 4, 3, "Плоский набор данных c наложенным мультипликативным Гауссовым шумом")

vis_poly(synth_data_sin, 4, 4, "Пример полиномиальной регрессии, полином 2 степени\nСинусоидный набор данных c наложенным Гауссовым шумом")

plt.subplots_adjust(wspace=0.155, top=0.88, bottom=0.6, left=0.017, right=0.98)
